...
export default function FoodDetailPage() {
  ...
  useEffect(() => {
    fetchFoodDetails();
    checkFavoriteStatus();
  }, [fetchFoodDetails, checkFavoriteStatus]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-20">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="space-y-4">
            <div className="relative h-96 rounded-lg overflow-hidden bg-white shadow-md">
              <Image src={food.image_url || "/food/ga1.jpg"} alt={food.name} fill className="object-cover" />
              {!food.is_available && (
            ...
            <div>
              ...
              {food.avg_rating && (
                <div className="flex items-center gap-2 mb-4">
                  <div className="flex items-center gap-1">{renderStars(Math.round(food.avg_rating))}</div>
                  <span className="text-lg font-semibold text-gray-700">{food.avg_rating.toFixed(1)}</span>
                  {food._count?.ratings && <span className="text-gray-500">({food._count.ratings} đánh giá)</span>}
                </div>
              )}

              {/* Price */}
              <div className="text-3xl font-bold text-orange-600 mb-4">{formatPrice(food.price)}</div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {food.stall_food_category.map((cat) => (
                      <span key={cat.id} className="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-sm">
                        {cat.category.name}
                      </span>
                    ))}
                  </div>
            ...
            {food.is_available && (
              <div className="bg-white rounded-lg p-6 shadow-md">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xl font-bold text-orange-600">{formatPrice(food.price * quantity)}</span>
                </div>
          ...
          <div className="bg-white rounded-lg p-6 shadow-md mb-8">
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-blue-600 hover:underline">Xem thêm món ăn từ quán này →</div>
              </div>
            </div>
          </div>
        )}
        ...
        {ratings.length > 0 && (
          <div className="bg-white rounded-lg p-6 shadow-md">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Đánh giá ({ratings.length})</h2>
            <div className="space-y-4">
