...
export default function StallDetailPage() {
  ...
  const fetchStallDetails = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const [stallResponse, foodsResponse] = await Promise.all([
        StallService.getById(stallId),
        FoodService.getByStallId(stallId),
      ]);
  ...
  }, [stallId, fetchStallDetails, checkFavoriteStatus]);
  ...
  const renderStars = (rating: number) => {
    return Array.from({ length: 5 }, (_, i) => (
      <StarIconSolid key={i} className={`w-4 h-4 ${i < rating ? "text-yellow-400" : "text-gray-300"}`} />
    ));
  };
  if (loading) {
    return (
          ...
          <div className="space-y-6">
            <div>
              <div className="flex items-start justify-between mb-2">
                ...
                <button onClick={handleToggleFavorite} className="p-2 rounded-full hover:bg-gray-100 transition-colors">
                  {isFavorite ? (
              ...
              <div className="flex items-center space-x-2 mb-4">
                <div className="flex items-center">{renderStars(Math.round(stall.avg_rating || 0))}</div>
                <span className="text-lg font-semibold text-gray-800">{stall.avg_rating?.toFixed(1) || "0.0"}</span>
                <span className="text-gray-500">({stall._count?.ratings || 0} đánh giá)</span>
              </div>
            </div>
            {/* Description */}
            {stall.description && (
                  ...
                  <div className="p-4">
                    ...
                    {food.avg_rating && (
                      <div className="flex items-center mt-2">
                        <div className="flex items-center mr-2">{renderStars(Math.round(food.avg_rating))}</div>
            ...
            <div className="space-y-4">
              {ratings.slice(0, 5).map((rating) => (
                <div key={rating.id} className="border-b border-gray-200 pb-4 last:border-b-0">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center">
                      {" "}
                      <div className="flex items-center mr-3">{renderStars(rating.score)}</div>
                      <span className="font-medium text-gray-800">{rating.user?.username || "Ẩn danh"}</span>
                    </div>
              ...
              {ratings.length > 5 && (
                <div className="text-center pt-4">
                  <button className="text-orange-600 hover:text-orange-700 font-medium">Xem thêm đánh giá</button>
                </div>
              )}
            </div>
          ) : (
