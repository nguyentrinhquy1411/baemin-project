...
function CartPageContent() {
  ...
  }, [cart]); // Update selection state when cart changes
  useEffect(() => {
    if (cartData.length === 0) {
      ...
      const existingItemIds = cartData.flatMap((stall) => stall.items.map((item: any) => item.id));
      const existingStallIds = cartData.map((stall) => stall.id);

      setSelectedItems((prevSelected) => {
        const newSelected = new Set(Array.from(prevSelected).filter((id) => existingItemIds.includes(id)));
        return newSelected;
      });

      setSelectedStalls((prevSelected) => {
        const newSelected = new Set(Array.from(prevSelected).filter((id) => existingStallIds.includes(id)));
        return newSelected;
      });
    }
  }, [cartData]);
  ...
  const handleItemSelect = (itemId: string, checked: boolean) => {
    ...
    cartData.forEach((stall) => {
      const allStallItemsSelected = stall.items.every((item: any) => newSelectedItems.has(item.id));
          ...
          <>
            ...
            <div className="flex flex-row fixed bottom-0 w-[90.6%] mr-16 h-16 bg-white items-center border-t">
              <div className="flex flex-row gap-2 w-1/2 h-full items-center ml-10">
                ...
                <div>Đã chọn:</div>
                <div>
                  {selectedItemCount} sản phẩm từ {selectedStalls.size} quán
                </div>
              </div>
