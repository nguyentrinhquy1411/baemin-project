...
export default function StallsPage() {
  ...
  const fetchCategoryAndStalls = useCallback(async () => {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-800">{category?.name || "Danh mục cửa hàng"}</h1>
              {category?.description && <p className="text-gray-600 mt-1">{category.description}</p>}
              <p className="text-sm text-gray-500 mt-1">{stalls.length} cửa hàng</p>
            </div>
          </div>
        </div>
        ...
        {stalls.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Chưa có cửa hàng nào</h3>
            <p className="text-gray-600">Hiện tại chưa có cửa hàng nào trong danh mục này.</p>
          </div>
        ) : (
          <>
                    ...
                    {stall.avg_rating && (
                      <div className="absolute top-3 right-3 bg-white rounded-full px-2 py-1 shadow-md">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm font-medium">{stall.avg_rating.toFixed(1)}</span>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    {stall.address && (
                      <div className="flex items-start gap-2 mb-2">
                        ...
                        <p className="text-sm text-gray-600 line-clamp-2">{stall.address}</p>
                      </div>
                    )}
                    ...
                    {stall._count && (
                      <div className="flex items-center justify-between text-sm text-gray-500">
                        ...
                        {stall._count.ratings > 0 && <span>{stall._count.ratings} đánh giá</span>}
                      </div>
                    )}
                  </div>
                </div>
