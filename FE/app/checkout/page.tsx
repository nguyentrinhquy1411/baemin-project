...
function CheckoutPageContent() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"momo" | "zalopay" | "credit_card" | "cash_on_delivery">(
    "cash_on_delivery"
  );
    ...
    if (user) {
      ...
      if (addresses.length === 0 && user) {
        const userAddress: SavedAddress = {
          id: "profile",
          name:
            user.first_name && user.last_name ? `${user.first_name} ${user.last_name}` : user.username || "Người dùng",
    ...
    <>
      ...
      <div className="px-16 flex flex-col gap-3 ">
        ...
        <div className="w-full bg-white rounded-md  flex flex-col pt-5 ">
          ...
          <div className="border-t w-full  mt-4">
            ...
            <div className="ml-[40%]  flex flex-row justify-between items-center py-2 ">
              <div className=" flex flex-row items-center gap-3">
                ...
                {selectedVoucher && <span className="text-sm text-green-600">({selectedVoucher.name})</span>}
              </div>
              <div
          ...
          <div className="border-t w-full  h-16 flex justify-end pr-5 gap-2 items-center">
            <span>Tổng số tiền ({cartItems.length} sản phẩm):</span>
            <span className="text-beamin font-bold">₫{subtotal.toLocaleString()}</span>
          </div>
        </div>
        <div className="w-full  flex flex-col bg-white rounded-md  pt-5 gap-3">
          ...
          <div className="w-full   border-t flex flex-col justify-end items-end pt-4  gap-4">
            <div className="flex justify-between w-[30%] ">
              ...
              <div className="text-sm mr-5">₫{subtotal.toLocaleString()}</div>
            </div>
            <div className="flex justify-between w-[30%] ">
              <div className="text-sm text-gray-900">Phí vận chuyển</div>
              <div className="text-sm mr-5">₫{shippingFee.toLocaleString()}</div>
            </div>
            <div className="flex justify-between w-[30%] ">
              <div className="text-sm text-gray-900">Tổng cộng Voucher giảm giá:</div>
              <div className="text-sm mr-5">-₫{discountAmount.toLocaleString()}</div>
            </div>
            <div className="flex justify-between w-[30%] ">
              <div className="text-sm text-gray-900">Tổng thanh toán</div>
              <div className="text-2xl mr-5 text-beamin">₫{total.toLocaleString()}</div>
            </div>
          </div>
      ...
      >
        <div className="space-y-4">
                        ...
                        <div className="flex justify-between items-start w-full">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              {address.isDefault && (
                                <span className="text-xs bg-beamin text-white px-2 py-1 rounded">Mặc định</span>
                              )}
                            </div>
          ...
          {/* Add New Address */}
          {!isAddingNewAddress ? (
            <Button type="dashed" icon={<PlusOutlined />} onClick={() => setIsAddingNewAddress(true)} block>
              Thêm địa chỉ mới
            </Button>
          ) : (
            <div className="border rounded-lg p-4 bg-gray-50">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <Button type="primary" onClick={handleAddNewAddress} className="bg-beamin">
                    Thêm địa chỉ
                  </Button>
                </div>
              </div>
          ...
          <div className="flex justify-end gap-2 pt-4">
            <Button onClick={() => setShowAddressModal(false)}>Hủy</Button>
            <Button
              type="primary"
        ...
        <div className="space-y-3 max-h-80 overflow-y-auto">
          <div
            className={`border p-3 rounded cursor-pointer hover:bg-gray-50 ${
              !selectedVoucher ? "border-blue-500 bg-blue-50" : "border-gray-300"
            }`}
